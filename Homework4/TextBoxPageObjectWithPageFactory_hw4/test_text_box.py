...
def test_invalid_email_validation(driver, invalid_email):
    ...
    assert page.is_email_error_present(), f"Не появилась ошибка валидации для email: {invalid_email}"
    assert output is None, f"Блок вывода появился с невалидным email: {invalid_email}"
